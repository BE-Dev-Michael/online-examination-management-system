...
function ExamCard(props) {
  ...
  return(
    <div className="flex flex-col items-center justify-between shadow-lg rounded-[1.2rem] w-[350px] md:w-full lg:w-full p-4 bg-white relative min-h-[240px] max-h-[240px]">
      ...
      <button className='bg-[#7B9EBE] hover:bg-[#6e8eac] rounded-xl text-white py-2 px-5 w-full mt-5 transition-all ease-linear delay-[.4ms]'>
        Open
      </button>
    </div>
  )
    ...
    const getExams = async () => {
        try {
            ...
            let fetchedExams = exams.data.map(data => {
                return { id: data._id, title: data.title, questions: data.questions, groups: data.groups }
            })
  ...
  return (
    ...
    <div className={`button-search flex flex-col ${isSidebarVisible ? 'md:flex-col' : 'md:flex-row md:justify-between'} gap-4 sm:justify-center lg:justify-between lg:flex-row items-center w-full px-5`}>
      <div className='max-w-[700px]'>
        <button className='relative flex justify-center items-center btn-create shadow-lg rounded-2xl px-7 py-4 font-semibold'>
    ...
    <div className="relative w-full h-full p-5 mb-24">
      ...
      {exams.map(exam => {
          return <ExamCard _id={exam.id} key={exam.id} title={exam.title} noOfQuestions={exam.questions.length + exam.groups.length}/>
      })}
      </div>
    </div> 
  </div>
