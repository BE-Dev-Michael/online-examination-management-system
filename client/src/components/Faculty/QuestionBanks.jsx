  ...
  return (
    <div className="absolute right-0 z-10 mt-2 w-44 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
      <div className="py-1 " role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
        <a ref={props.editTitle} onClick={editBankTitle} className="flex items-center block cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
...
function EditBankMode() {
  ...
  const submitEditedBank = async (e) => {
    ...
    try {
        ...
        //* Naguupdate ng array of objects
...
function QuestionBanks() {
  ...
  return (
    <>
      ...
      <div className="relative w-full h-full p-5 mb-2 overflow-y-scroll">
        <div class="grid grid-rows-2 gap-10 md:grid-cols-2 lg:grid-cols-3 justify-center items-center ease-in-out duration-300">
        {banks.map(bank => {
            return <Bank _id={bank.id} open={() => openBank(bank.id)} key={bank.id} title={bank.title} noOfQuestions='17'/>
        })}
        </div>
      </div> 
